...
const OPENROUTER_API_KEY = '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';
const LIMIT = 5;
const MAX_QUIZ_TIME = 10 * 60 * 1000;
const CAROUSEL_INTERVAL = 5000;
const CAROUSEL_RESUME_DELAY = 10000;
const OPENROUTER_MODEL = 'openrouter/andromeda-alpha';
...
async function translate(text, from = 'EN', to = 'FR') {
  ...
  try {
    const response = await fetch('https://api-inference.huggingface.co/models/Helsinki-NLP/opus-mt-' + (from === 'EN' ? 'en-fr' : 'fr-en'), {
...
async function loadBooks(translation) {
  ...
  if (bookCache['EN']) {
    // Utiliser le cache en anglais, mais afficher les noms selon la langue
    populateBooks(bookCache['EN'], translation);
    return;
  }
  try {
    const data = await fetch(`https://bible-api.com/data/web`).then(res => res.json());
    bookCache['EN'] = data.books; // Toujours stocker en anglais
...
async function loadVerses(translation, bookId, chapter) {
  ...
  if (chapterCache[cacheKey]?.['en']) {
    // Utiliser les données en anglais depuis le cache et traduire si nécessaire
    const verses = chapterCache[cacheKey]['en'];
    const translatedVerses = translation === 'FR' ? await Promise.all(verses.map(async v => ({
      ...v,
      text: await translate(v.text, 'EN', 'FR')
    }))) : verses;
    chapterCache[cacheKey][translation.toLowerCase()] = translatedVerses;
...
async function showHighlightedChapter(translation, bookId, chapter, selectedVerse = null, updateHighlightOnly = false) {
  ...
  if (!verses && chapterCache[cacheKey]?.['en']) {
    // Utiliser les données en anglais depuis le cache et traduire
    const rawVerses = chapterCache[cacheKey]['en'];
    ...
    try {
      ...
      chapterCache[cacheKey]['en'] = rawVerses;
      chapterCache[cacheKey][translation.toLowerCase()] = verses;
    } catch (error) {
...
async function displayPage(page, lang) {
  ...
  const translations = await Promise.all(pageData.map(item => translate(item.preview, 'EN', lang)));
  pageData.forEach((item, i) => {
    const div = document.createElement('div');
    div.className = 'verse';
    const bookId = item.title.match(/^([A-Z0-9]+)/)?.[1] || item.book_id;
    const reference = lang === 'FR' ? item.title.replace(/(\d+[A-Za-z]*)/, frenchBookNames[bookId] || bookId).replace(/\./g, ':') : item.title.replace(/\./g, ':');
...
async function fetchPage(query, page, lang) {
  const cacheKey = `${query}-EN`; // Toujours stocker les données brutes en anglais
  if (searchCache[cacheKey]?.[page]) {
    cachedResults[page] = searchCache[cacheKey][page];
    await displayPage(page, lang);
    return;
  }
  ...
  document.getElementById('searchNavBottom').classList.remove('show');
...
async function generateQuestion(verse) {
  ...
  const usedDistractorsList = Array.from(usedDistractors).join(', ');
  const userPrompt = isFrench ?
    `Voici un verset biblique : "${verseText}" (${fullRef}). Générez une seule question à choix multiple (QCM) en français basée sur ce verset. La question doit se concentrer sur un détail spécifique du verset (par exemple, un personnage, un lieu, un événement ou un thème). Fournissez exactement 4 options uniques, avec une seule correcte. Les trois distracteurs doivent être plausibles, dérivés d'autres parties de la Bible, et strictement liés au contexte du verset (par exemple, même période, livre ou thème similaire). Aucune des options (y compris la réponse correcte) ne doit apparaître dans la liste suivante : ${usedDistractorsList || 'aucun'}. Répondez uniquement avec un objet JSON : {"question": "...", "options": ["...", "...", "...", "..."], "correctAnswer": 0 ou 1 ou 2 ou 3}` :
    `Here is a Bible verse: "${verse.text}" (${fullRef}). Generate a single multiple-choice question (QCM) based on this verse. The question must focus on a specific detail from the verse (e.g., a character, place, event, or theme). Provide exactly 4 unique options, with only one correct. The three distractors must be plausible, derived from other parts of the Bible, and strictly related to the verse’s context (e.g., same period, book, or similar theme). None of the options (including the correct answer) must appear in the following list: ${usedDistractorsList || 'none'}. Respond only with a JSON object: {"question": "...", "options": ["...", "...", "...", "..."], "correctAnswer": 0 or 1 or 2 or 3}`;
    ...
    messages: [
      {
        role: 'system',
        content: isFrench ?
          'Vous êtes un assistant qui génère des questions à choix multiple en français. Répondez uniquement avec un objet JSON contenant une question, exactement quatre options uniques, et l\'index de la réponse correcte : {"question": "...", "options": ["...", "...", "...", "..."], "correctAnswer": 0 ou 1 ou 2 ou 3}. Les options doivent être pertinentes par rapport au verset donné et ne doivent pas se répéter à travers le quiz.' :
          'You are a helpful assistant that generates multiple-choice questions. Respond only with a JSON object containing a question, exactly four options, and the index of the correct answer: {"question": "...", "options": ["...", "...", "...", "..."], "correctAnswer": 0 or 1 or 2 or 3}. The options must be relevant to the given verse and must not repeat across the quiz.',
      },
  ...
  try {
    ...
    if (qcm.options.some(opt => usedDistractors.has(opt))) {
      throw new Error('Duplicate options detected');
    }
    qcm.options.forEach(opt => usedDistractors.add(opt));
    ...
    const fallback = {
      question: isFrench ?
        `Qui est mentionné dans ${fullRef} ?` :
        `Who is mentioned in ${fullRef}?`,
      options: [
        isFrench ? 'Sibbecai' : 'Sibbecai',
        isFrench ? 'David' : 'David',
        isFrench ? 'Salomon' : 'Solomon',
        isFrench ? 'Moïse' : 'Moses'
      ],
      correctAnswer: 0,
      verse: { ...verse, fullRef }
    };
    if (fallback.options.some(opt => usedDistractors.has(opt))) {
      fallback.options = [
        isFrench ? 'Sibbecai' : 'Sibbecai',
        isFrench ? 'Goliath' : 'Goliath',
        isFrench ? 'Saül' : 'Saul',
        isFrench ? 'Élie' : 'Elijah'
      ];
    }
    fallback.options.forEach(opt => usedDistractors.add(opt));
    return fallback;
  }
}
...
async function generateQuiz() {
  ...
  const questionPromises = verses.map(verse => generateQuestion(verse).then(q => {
    ...
    const fallback = {
      question: quizLang === 'FR' ? `Qui est mentionné dans ${fullRef} ?` : `Who is mentioned in ${fullRef}?`,
      options: [
        isFrench ? 'Sibbecai' : 'Sibbecai',
        isFrench ? 'Goliath' : 'Goliath',
        isFrench ? 'Saül' : 'Saul',
    ...
    if (fallback.options.some(opt => usedDistractors.has(opt))) {
      fallback.options = [
        isFrench ? 'Sibbecai' : 'Sibbecai',
        isFrench ? 'Jonathan' : 'Jonathan',
        isFrench ? 'Absalom' : 'Absalom',
        isFrench ? 'Ésaü' : 'Esau'
      ];
    }
    fallback.options.forEach(opt => usedDistractors.add(opt));
...
const debouncedSearchPrev = debounce(() => {
  if (currentPage > 0) {
    currentPage--;
    displayPage(currentPage, currentLang);
  }
}, 300);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('translation').addEventListener('change', async (e) => {
    ...
    // Recharger les livres avec la nouvelle langue, en conservant les données en anglais
    ...
    if (currentQuery) {
      await displayPage(currentPage, currentLang);
    } else {
